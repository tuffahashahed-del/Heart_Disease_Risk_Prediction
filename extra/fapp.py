...
st.image("image/aiheart.jpg",use_container_width=True)

st.markdown("""<style>.stApp {background-color:Azure;}</style>""",
    unsafe_allow_html=True)
st.write('''
<h2 style="color:#21618c;font-size:16px;">
...
with c1:
    ...
    st.markdown('<h1 style="color:SteelBlue;font-size:16px; text-align:left;">Resting Blood Pressure🩺</h1>', unsafe_allow_html=True)
    resting=st.number_input("",80,200,120)
with c2:
    st.markdown('<h1 style="color:SteelBlue;font-size:16px; text-align:left;">Cholestrol🧪 </h1>', unsafe_allow_html=True)
...
with c3:
    st.markdown('<h1 style="color:SteelBlue;font-size:16px; text-align:left;">Exercise_Angina🏃🏻</h1>', unsafe_allow_html=True)
